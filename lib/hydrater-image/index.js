...
var generateThumb = function(path, thumbSize, cb) {
  ...
  shellExec('convert ' + path + " -auto-orient -thumbnail " + thumbSize + "\\> png:fd:1 | base64 -i -w 0", function(err, stdout, stderr) {
    if(err) {
      return cb(err);
    }
...
var generateDisplay = function(path, displaySize, cb) {
  ...
  shellExec('convert ' + path + " -auto-orient -resize " + displaySize + "\\> fd:1 | base64 -i -w 0", function(err, stdout, stderr) {
    if(err) {
      return cb(err);
    }
  ...
  async.auto({
    generateThumb: function(cb) {
      generateThumb(path, config.thumb_size, function(err, thumbnail) {
        ...
        document.metadatas.thumb = "data:image/png;base64," + thumbnail;
        cb();
      });
    },
    generateDisplay: function(cb) {
      generateDisplay(path, config.display_size, function(err, display) {
        ...
        document.metadatas.display = "data:" + document.metadatas['content-type'] + ";base64," + display;
        cb();
      });
    },
  }, function(err) {
