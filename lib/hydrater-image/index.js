...
var generateThumb = function(path, thumbSize, cb) {
  ...
  shellExec('convert ' + path + " -auto-orient -thumbnail " + thumbSize + "\\> png:fd:1 | base64 -i -w 0", function(err, stdout) {
    if(err) {
      return cb(err);
    }
...
var generateDisplay = function(path, displaySize, cb) {
  ...
  shellExec('convert ' + path + " -auto-orient -resize " + displaySize + "\\> -quality 80 jpg:fd:1 | base64 -i -w 0", {maxBuffer: 3000 * 1024}, function(err, stdout) {
    if(err) {
      return cb(err);
    }
  ...
  async.auto({
    ...
    generateDisplay: function(cb) {
      generateDisplay(path, config.display_size, function(err, display) {
        ...
        document.metadatas.display = "data:image/jpeg;base64," + display;
        cb();
      });
    },
  }, function(err) {
