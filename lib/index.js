...
var generateThumb = function(path, thumbSize, cb) {
  ...
  shellExec('convert -limit memory 190MiB -limit map 30MiB -limit area 10Mib ' + path + " -auto-orient -thumbnail " + thumbSize + "\\> -strip png:fd:1 | base64 -i -w 0", {maxBuffer: 20480 * 1024}, cb);
};
...
var generateDisplay = function(path, displaySize, cb) {
  ...
  shellExec('convert -limit memory 190MiB -limit map 30MiB -limit area 10Mib ' + path + " -auto-orient -resize " + displaySize + "\\> -quality 80 jpg:fd:1 | base64 -i -w 0", {maxBuffer: 20480 * 1024}, cb);
};
