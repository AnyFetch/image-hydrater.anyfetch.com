...
var generateThumb = function(path, thumbSize, cb) {
  ...
  shellExec('convert ' + path + " -auto-orient -thumbnail " + thumbSize + "\\> -strip png:fd:1 -limit memory 200MiB -limit map 400MiB | base64 -i -w 0", cb);
};
...
var generateDisplay = function(path, displaySize, cb) {
  ...
  shellExec('convert ' + path + " -auto-orient -resize " + displaySize + "\\> -quality 80 jpg:fd:1 -limit memory 200MiB -limit map 400MiB | base64 -i -w 0", {maxBuffer: 20480 * 1024}, cb);
};
