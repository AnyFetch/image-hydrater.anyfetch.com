  ...
  async.series({
    generateThumb: function(cb) {
      ...
      generateThumb(path, config.thumb_size, function(err, thumbnail) {
        ...
        if(!thumbnail) {
          return cb(new hydrationError("Unable to hydrate image and create thumbnail"), changes);
        }

        changes.datas.thumb = "data:image/png;base64," + thumbnail;
        cb();
      });
    },
    generateDisplay: function(cb) {
      ...
      generateDisplay(path, config.display_size, function(err, display) {
        ...
        if(!display) {
          return cb(new hydrationError("Unable to hydrate image and create display"), changes);
        }

        changes.datas.display = "data:image/jpeg;base64," + display;
        cb();
      });
    },
  }, function(err) {
