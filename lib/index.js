...
module.exports = function(path, document, changes, finalCb) {
  // Skip PSD
  if (document.metadata && document.metadata.path && (document.metadata.path.indexOf('.psd') !== -1 || document.metadata.path.indexOf('.PSD') !== -1)) {
  ...
  async.series({
    generateThumb: function(cb) {
      ...
      generateThumb(path, config.thumb_size, function(err, thumbnail) {
        ...
        if(!thumbnail) {
          return cb(new hydrationError("Unable to hydrate image and create thumbnail"));
        }
    ...
    generateDisplay: function(cb) {
      ...
      generateDisplay(path, config.display_size, function(err, display) {
        ...
        if(!display) {
          return cb(new hydrationError("Unable to hydrate image and create display"));
        }
