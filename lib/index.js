...
var generateThumb = function(path, thumbSize, cb) {
  ...
  shellExec('convert -limit memory 100MiB -limit map 30MiB -limit area 10Mib ' + path + " -auto-orient -thumbnail " + thumbSize + "\\> -strip png:fd:1 | base64 -i -w 0", {maxBuffer: 20480 * 1024, timeout: 45000, killSignal: 'SIGKILL'}, cb);
};
...
var generateDisplay = function(path, displaySize, cb) {
  ...
  shellExec('convert -limit memory 100MiB -limit map 30MiB -limit area 10Mib ' + path + " -auto-orient -resize " + displaySize + "\\> -quality 80 jpg:fd:1 | base64 -i -w 0", {maxBuffer: 20480 * 1024, timeout: 45000, killSignal: 'SIGKILL'}, cb);
};
  ...
  async.series({
    generateThumb: function(cb) {
      ...
      generateThumb(path, config.thumb_size, function(err, thumbnail) {
        ...
        changes.data.thumb = "data:image/png;base64," + thumbnail;
        cb();
      });
    },
